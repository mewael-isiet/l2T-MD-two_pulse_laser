  ...
  Fix(lmp, narg, arg)
{
  ...
  // fix 1 all laser I0 1.0 R 0.1 Lp 50.0 t0 0.50 sigma0 0.1 x 0.0
  if (narg != 15) error->all(FLERR,"Illegal fix laser command");
...
void FixLaser::init_list(int id, NeighList *ptr)
{
  list = ptr;
}
...
void FixLaser::initial_integrate(int vflag)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
	    ...
	    if(dim_flag == 0) mxe_temperature[i] += (0.9394372787*F*(1-R)/(Lp*t0*Ce[i]))*exp(-((x[i][0] - global_min_x)/(global_max_x - global_min_x)-c1)/(Lp/(global_max_x - global_min_x)))*exp(-2.7725887222*(time-2*t0)*(time-2*t0)/(t0*t0))*update->dt;
            if(dim_flag == 1) mxe_temperature[i] += (0.9394372787*F*(1-R)/(Lp*t0*Ce[i]))*exp(-((x[i][1] - global_min_y)/(global_max_y - global_min_y)-c1)/(Lp/(global_max_y - global_min_y)))*exp(-2.7725887222*(time-2*t0)*(time-2*t0)/(t0*t0))*update->dt;
            if(dim_flag == 2) mxe_temperature[i] += (0.9394372787*F*(1-R)/(Lp*t0*Ce[i]))*exp(-((x[i][2] - global_min_z)/(global_max_z - global_min_z)-c1)/(Lp/(global_max_z - global_min_z)))*exp(-2.7725887222*(time-2*t0)*(time-2*t0)/(t0*t0))*update->dt;
    }
  }
}
/* ----------------------------------------------------------------------
