...
enum{TYPE,TYPE_FRACTION,MOLECULE,X,Y,Z,CHARGE,MASS,SHAPE,LENGTH,TRI,
     ...
     SMD_CONTACT_RADIUS,DPDTHETA,INAME,DNAME,VX,VY,VZ,TEMP,FREQ,ENT,FREQF,ENTF,XI};
...
void Set::set(int keyword)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    else if (keyword == TEMP) atom->mxe_temperature[i] = dvalue;
    else if (keyword == FREQ) atom->frequency[i] = dvalue;
    else if (keyword == ENT) atom->entropy[i] = dvalue;
    else if (keyword == FREQF) atom->freq_force[i] = dvalue;
    else if (keyword == ENTF) atom->entropy_force[i] = dvalue;
    else if (keyword == XI) {
      atom->xi[i][0] = xvalue;
      atom->xi[i][1] = yvalue;
      atom->xi[i][2] = zvalue;
