...
enum{ID,MOL,PROC,PROCP1,TYPE,ELEMENT,MASS,
     ...
     TQX,TQY,TQZ,
     COMPUTE,FIX,VARIABLE,INAME,DNAME,TEMP,FREQ,ENT,FREQF,ENTF,
     XI0, XI1, XI2, FXI0, FXI1, FXI2};
  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      // USER-MXE package
        ...
        if (!atom->mxe_temperature_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = atom->mxe_temperature;
        nstride = 1;
      } else if (thresh_array[ithresh] == FREQ) {
        if (!atom->frequency_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = atom->frequency;
        nstride = 1;
      } else if (thresh_array[ithresh] == ENT) {
        if (!atom->entropy_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = atom->entropy;
        nstride = 1;
      } else if (thresh_array[ithresh] == FREQF) {
        if (!atom->freq_force_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = atom->freq_force;
        nstride = 1;
      } else if (thresh_array[ithresh] == ENTF) {
        if (!atom->entropy_force_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = atom->entropy_force;
        nstride = 1;
      }	else if (thresh_array[ithresh] == XI0) {
        if (!atom->xi_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->xi[0][0];
        nstride = 3;
      } else if (thresh_array[ithresh] == XI1) {
        if (!atom->xi_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->xi[0][1];
        nstride = 3;
      } else if (thresh_array[ithresh] == XI2) {
        if (!atom->xi_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->xi[0][2];
        nstride = 3;
      }	else if (thresh_array[ithresh] == FXI0) {
        if (!atom->fxi_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->fxi[0][0];
        nstride = 3;
      } else if (thresh_array[ithresh] == FXI1) {
        if (!atom->fxi_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->fxi[0][1];
        nstride = 3;
      } else if (thresh_array[ithresh] == FXI2) {
        if (!atom->fxi_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->fxi[0][2];
...
int DumpCustom::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 0; iarg < narg; iarg++) {
    ...
    } else if (strcmp(arg[iarg],"mxe_temperature") == 0) {
      if (!atom->mxe_temperature_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_mxe_temperature;
      vtype[i] = DOUBLE;
    }  else if (strcmp(arg[iarg],"frequency") == 0) {
      if (!atom->frequency_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_frequency;
      vtype[i] = DOUBLE;
    }  else if (strcmp(arg[iarg],"entropy") == 0) {
      if (!atom->entropy_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_entropy;
      vtype[i] = DOUBLE;
    }  else if (strcmp(arg[iarg],"freq_force") == 0) {
      if (!atom->freq_force_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_freq_force;
      vtype[i] = DOUBLE;
    }  else if (strcmp(arg[iarg],"entropy_force") == 0) {
      if (!atom->entropy_force_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_entropy_force;
      vtype[i] = DOUBLE;
    }  else if (strcmp(arg[iarg],"xi0") == 0) {
      if (!atom->xi_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_xi0;
      vtype[i] = DOUBLE;
    }  else if (strcmp(arg[iarg],"xi1") == 0) {
      if (!atom->xi_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_xi1;
      vtype[i] = DOUBLE;
    }  else if (strcmp(arg[iarg],"xi2") == 0) {
      if (!atom->xi_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_xi2;
      vtype[i] = DOUBLE;
    }  else if (strcmp(arg[iarg],"fxi0") == 0) {
      if (!atom->fxi_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_fxi0;
      vtype[i] = DOUBLE;
    }  else if (strcmp(arg[iarg],"fxi1") == 0) {
      if (!atom->fxi_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_fxi1;
      vtype[i] = DOUBLE;
    }  else if (strcmp(arg[iarg],"fxi2") == 0) {
      if (!atom->fxi_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_fxi2;
      vtype[i] = DOUBLE;
...
int DumpCustom::modify_param(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"thresh") == 0) {
    ...
    else if (strcmp(arg[1],"mxe_temperature") == 0) thresh_array[nthresh] = TEMP;
    else if (strcmp(arg[1],"frequency") == 0) thresh_array[nthresh] = FREQ;
    else if (strcmp(arg[1],"entropy") == 0) thresh_array[nthresh] = ENT;
    else if (strcmp(arg[1],"freq_force") == 0) thresh_array[nthresh] = FREQF;
    else if (strcmp(arg[1],"entropy_force") == 0) thresh_array[nthresh] = ENTF;
    else if (strcmp(arg[1],"xi0") == 0) thresh_array[nthresh] = XI0;
    else if (strcmp(arg[1],"xi1") == 0) thresh_array[nthresh] = XI1;
    else if (strcmp(arg[1],"xi2") == 0) thresh_array[nthresh] = XI2;
    else if (strcmp(arg[1],"fxi0") == 0) thresh_array[nthresh] = FXI0;
    else if (strcmp(arg[1],"fxi1") == 0) thresh_array[nthresh] = FXI1;
    else if (strcmp(arg[1],"fxi2") == 0) thresh_array[nthresh] = FXI2;
    // end USER-MXE package
...
void DumpCustom::pack_mxe_temperature(int n)
{
  double *mxe_temperature = atom->mxe_temperature;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = mxe_temperature[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_frequency(int n)
{
  double *frequency = atom->frequency;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = frequency[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_entropy(int n)
{
  double *entropy = atom->entropy;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = entropy[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_freq_force(int n)
{
  double *freq_force = atom->freq_force;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = freq_force[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_entropy_force(int n)
{
  double *entropy_force = atom->entropy_force;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = entropy_force[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_xi0(int n)
{
  double **xi = atom->xi;
...
void DumpCustom::pack_xi1(int n)
{
  double **xi = atom->xi;
...
void DumpCustom::pack_xi2(int n)
{
  double **xi = atom->xi;
...
void DumpCustom::pack_fxi0(int n)
{
  double **fxi = atom->fxi;
...
void DumpCustom::pack_fxi1(int n)
{
  double **fxi = atom->fxi;
...
void DumpCustom::pack_fxi2(int n)
{
  double **fxi = atom->fxi;
